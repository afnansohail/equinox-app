...
const vercelApiUrl =
  (Constants.expoConfig?.extra as any)?.vercelApiUrl || process.env.EXPO_PUBLIC_VERCEL_API_URL;
...
const mapDbStock = (row: DbStockRow): Stock => ({
  ...
  marketCap: row.market_cap !== undefined ? Math.trunc(toNumber(row.market_cap)) : undefined,
  high52Week: row.high_52week !== undefined ? toNumber(row.high_52week) : undefined,
  low52Week: row.low_52week !== undefined ? toNumber(row.low_52week) : undefined,
  lastUpdated: row.last_updated ?? new Date().toISOString(),
});

const isRecent = (timestamp: string | null | undefined, maxAgeMinutes = 30): boolean => {
  ...
  try {
    ...
    if (tx.transactionType === 'BUY') {
      if (holding) {
        ...
        const newTotalInvested = Number(holding.total_invested) + tx.totalAmount;
...
export async function refreshStockPrices(symbols: string[]): Promise<void> {
  ...
  const response = await axios.post(`${vercelApiUrl}/api/scrape-all-stocks`, { symbols });
